...
public class FfmpegService {
    ...
    private String[] cmd(Path input, Path output) {
        return new String[]{
                ...
                "-f", "hls",
                output.toAbsolutePath() + "/720p/slice.m3u8",
                // 1080p
                ...
                "-start_number", "0", "-hls_time", "1", "-hls_list_size", "0",
                "-f", "hls",
                output.toAbsolutePath() + "/1080p/slice.m3u8"
        };
    }
    ...
    public void thumbnail(Path input, Path output) {
        ...
        if (!value.startsWith("duration=")) {
            throw new RuntimeException("thumbnail failed:" + value);
        }
    ...
    private static void execute(Consumer<String> consumer, String... cmds) {
        ...
        try {
            ...
            if (0 != process.waitFor()) {
                throw new RuntimeException("get failed: " + value);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
